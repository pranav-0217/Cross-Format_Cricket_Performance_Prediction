...
def scrape_cricket_data(classs, page, type):
    ...
    for row in rows[1:]:  # Skip the header row
        ...
        try:
            ...
            a_tag = cols[0].find('a')
            player_link = a_tag['href'] if (a_tag and a_tag.has_attr('href')) else None
            player_url = f"https://www.espncricinfo.com{player_link}" if player_link else ''
            ...
            print(f"Warning: skipping row on page {page} — expected enough cols, got {len(cols)}. Row texts: {row_texts}")
            continue
